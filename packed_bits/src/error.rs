...
pub enum PackedBitsError {
    #[cfg_attr(feature = "std", error("Bit width N must be in the range 1..=32, got {0}"))]
    ...
    #[cfg_attr(feature = "std", error("N mismatch: expected {expected}, found {found}"))]
...
impl core::fmt::Display for PackedBitsError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            PackedBitsError::InvalidBitWidth(n) => write!(f, "Bit width N must be in the range 1..=32, got {}", n),
            PackedBitsError::ValueOverflow(v, n) => write!(f, "Value {} does not fit in {} bits", v, n),
            PackedBitsError::IndexOutOfBounds(i, l) => write!(f, "Index {} is out of bounds for length {}", i, l),
            PackedBitsError::InsufficientBytes(n) => write!(f, "Insufficient bytes for {} elements", n),
            PackedBitsError::InvalidMagic => write!(f, "invalid magic bytes in storage"),
            PackedBitsError::InvalidN { expected, found } => write!(f, "N mismatch: expected {}, found {}", expected, found),
