...
fn main() -> Result<()> {
    ...
    // Method 1: Using try_parse_iter
    println!("\n1. Using try_parse_iter:");
    let packets: Vec<Packet> = try_parse_iter::<Packet, 12>(&data)
        .collect::<Result<Vec<_>, _>>()?;
    
    for packet in &packets {
        println!("  Packet {}: ts={}, val={}, flags={:04x}",
                 packet.id, packet.timestamp, packet.value, packet.flags);
    }

    // Method 2: Using batch for fixed-size groups
    println!("\n2. Using batch for u32 groups:");
    if let Some(first_three) = batch::<3, u32>(
        bytemuck::cast_slice(&data[0..12]), 0
    ) {
        println!("  First 3 u32s: {:?}", first_three);
    }

    // Method 3: Using SliceParseExt
    ...
    // Method 4: iter_parse for stateful parsing
    println!("\n4. Using iter_parse with state:");
    let mut pos = 0;
    let packets_iter: Vec<Packet> = iter_parse(|| {
        if pos + 12 <= data.len() {
            let packet = Packet::try_from(&data[pos..pos + 12]).ok()?;
            pos += 12;
            Some(packet)
        } else {
            None
    ...
    println!("  Parsed {} packets via iter_parse", packets_iter.len());
