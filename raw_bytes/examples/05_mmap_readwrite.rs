...
fn create_initial_scores(path: &Path) -> std::io::Result<()> {
    let scores = vec![
        GameScore { player_id: 1, score: 1000, level: 5, flags: 0 },
        GameScore { player_id: 2, score: 2500, level: 8, flags: 1 },
        GameScore { player_id: 3, score: 500, level: 2, flags: 0 },
        GameScore { player_id: 4, score: 3200, level: 10, flags: 3 },
    ];
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[cfg(all(feature = "std", feature = "mmap"))]
    {
        ...
        for (_i, score) in c.iter().enumerate() {
            println!("  Player {}: score={}, level={}, flags={}",
